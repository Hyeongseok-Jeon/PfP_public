...
class ArgoverseForecastingLoader:
    ...
    def get_time_list(self) -> np.ndarray:
        time_list = self.seq_df["TIMESTAMP"].to_numpy()
        time_list = np.unique(time_list)
        return time_list
        
        
        
    def traj_by_id(self, id, time_list) -> np.ndarray:
        """Get the trajectory for the track of type 'AGENT' in the current sequence.

        Returns:
            numpy array of shape (seq_len x 2) for the agent trajectory
        """
        local_data = self.seq_df[self.seq_df["TRACK_ID"] == id]
        init_time = local_data["TIMESTAMP"][local_data.index[0]]
        end_time = local_data["TIMESTAMP"][local_data.index[-1]]
        init_index = np.where(time_list == init_time)[0][0]
        end_index = np.where(time_list == end_time)[0][0]
        
        agent_traj = np.zeros((end_index - init_index + 1, 2))
        for i in range(init_index, end_index + 1):
            time = time_list[i]
            agent_x = local_data[local_data["TIMESTAMP"]==time]["X"]
            agent_y = local_data[local_data["TIMESTAMP"]==time]["Y"]
            if len(agent_x)==0:
                agent_traj[i-init_index:i-init_index+1,:] = np.asarray([[0,0]])
            else:
                agent_traj[i-init_index:i-init_index+1,:]=np.column_stack((agent_x, agent_y))
        return agent_traj, [init_index, end_index]
